...
import Data.Array.IArray (Array, assocs, listArray, elems, (!), (//))
import Data.List (find)

data Board a = Board (Array Int (Maybe a)) deriving (Eq)
...
emptyCells (Board array) = foldr (\a -> if isNothing a then (1+) else id) 0 array
...
play card board = 
  case findFirstEmpty board of
  Just i -> board `update` [(i, Just card)]
  Nothing -> board

findFirstEmpty :: Board a -> Maybe Int
findFirstEmpty (Board array) = 
  case firstEmptyAssoc of
  Nothing -> Nothing
  Just (i, v) -> Just i
